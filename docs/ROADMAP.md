...
- [ ] **Task 020: 홈 화면 신설 및 로그인 리다이렉트 정리, 전체 한국어화** 🔴 — 우선순위
  - [ ] 홈(`/`)을 이벤트 목록(`/events`)과 별개인 서비스 소개/환영 화면으로 신설 (`(mobile)` 라우트 그룹, 스타터킷 데모 대체)
  - [ ] 로그인/회원가입/비번변경/OAuth 콜백 성공 리다이렉트를 `/protected`(스타터킷 기본값)에서 홈(`/`)으로 변경
  - [ ] `/protected` 스타터킷 데모 페이지 및 빈 라우트 폴더 잔재 정리
  - [ ] 스타터킷 전용 미사용 컴포넌트(Hero, DeployButton 등) 정리
  - [ ] 전체 UI 텍스트 한국어화 (`app/layout.tsx` lang/metadata 포함)
  - [ ] 테스트 체크리스트: 미로그인/로그인 상태 홈 분기 노출 / 로그인 성공 후 `/` 이동 / `/events` 기존 플로우 회귀 확인

- [ ] **Task 007: 이벤트 생성/수정 폼 저장 로직 연동** 🔴
