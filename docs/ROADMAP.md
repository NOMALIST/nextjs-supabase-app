...
> **선행 조건**: Phase 7 변경사항(Task 021~026)이 미커밋 상태로 남아있어 4개 커밋으로 정리 후 착수(마이그레이션/타입 → 이벤트·RSVP 실연동 → 관리자 인증/실연동 → 문서).

- [x] **Task 015: 전체 플로우 통합 E2E 테스트** 🟢
  - [x] 주최자 여정: 회원가입 → 이벤트 생성(slug 발급) → 공유 링크 복사 → 정산 입력 → 입금 체크 → 복제 → 수정
  - [x] 참여자 여정: 무인증 slug 접속 → RSVP 제출(참여/불참, 이름 누락 검증) → 정원 초과 배지 → 정산 요약 공개 노출
  - [x] 관리자 여정: 관리자 로그인 → 대시보드/이벤트/사용자/통계 4페이지가 위 여정에서 생성한 실데이터 반영 확인
  - [x] 접근 매트릭스: 비로그인·일반회원·관리자 3역할 × admin 4페이지 전부 기대대로 동작
  - [x] 에러/엣지 케이스: 타 주최자 이벤트 직접 접근 404, 잘못된 slug 404, 정원 null/확정 인원 0명 처리 정상
  - [x] 검증 중 발견한 버그 수정: `components/event-dashboard.tsx`의 총비용(`total_cost`) 입력이 로컬 state에만 저장되고 DB에 반영되지 않던 문제 → `onBlur` 시 `events` UPDATE하도록 수정(공개 페이지 "정산 미확정" 표시 오류의 원인이었음)
  - [x] 검증 중 발견한 회귀 수정: 홈(`/`) 페이지가 Task 023의 Suspense 분리 패턴 적용 전 상태로 남아있어 Next.js 16 blocking-route 경고 발생 → 다른 페이지와 동일한 async 컨텐츠 컴포넌트 + Suspense 패턴 적용
  - [x] `get_advisors` 최종 점검: performance 0건, security 2건은 모두 기존 설계(익명 RSVP INSERT 허용, leaked password protection 미사용)로 신규 아님
  - [x] 테스트 데이터 정리: MVP 단계로 실운영 데이터 없어 별도 정리 없이 유지 결정

- [x] **Task 016: 최적화 및 배포** 🟢
  - [x] `npm run check-all` 통과, `npm run build` 프로덕션 빌드 통과
  - [x] 캐싱/재검증 전략: 현행 유지(명시적 `revalidate`/`dynamic` 미지정) 결정 — 참여 현황·정원·정산·관리자 통계 등 실시간성이 중요한 도메인 특성상 캐싱을 걸지 않는 편이 현재는 올바른 선택이라 판단, 코드 변경 없이 이 판단 근거만 문서화
  - [x] 스타터킷 데모 잔존 정리: `README.md`를 프로젝트 고유 설명으로 재작성, `lib/utils.ts`의 튜토리얼 잔재 주석 제거. Hero/DeployButton 등은 Task 020에서 이미 정리 확인. `docs/guides/`는 CLAUDE.md의 기존 경고 유지로 충분하다고 판단해 그대로 둠
  - [x] `.playwright-mcp/` 세션 산출물을 `.gitignore`에 추가(매 Playwright MCP 실행마다 재생성되어 `format:check`를 반복적으로 막던 문제의 근본 해결)
...
**최종 수정**: 2026-07-09 (Phase 8 완료 — 전체 플로우 통합 E2E 검증(정산 저장 버그·홈 화면 회귀 발견 및 수정 포함), check-all/build 통과, 캐싱 전략 현행 유지 결정, README 재작성 등 배포 준비 마무리. MVP 개발 로드맵 전체 완료)
